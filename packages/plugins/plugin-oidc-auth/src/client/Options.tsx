            ...
            basic: {
              ...
              properties: {
                ...
                tokenEndpointAuthMethod: {
                  type: 'string',
                  title: '{{t("Token endpoint authentication")}}',
                  'x-component': 'Select',
                  'x-decorator': 'FormItem',
                ...
                redirectUri: {
                  ...
                  'x-decorator': 'FormItem',
                  'x-decorator-props': {
                    tooltip: '{{t("Leave blank to derive it from the current NocoBase URL.")}}',
                  },
                },
              ...
              properties: {
                ...
                userBindField: {
                  ...
                  enum: [
                    ...
                    { label: lang('Do not bind existing users'), value: 'none' },
                  ],
                  required: true,
                },
                bindExistingUserByEmail: {
                  type: 'boolean',
                  title: '{{t("Bind existing users by email")}}',
                  'x-component': 'Checkbox',
                  'x-decorator': 'FormItem',
                  default: true,
                },
                trustedEmailDomains: {
                  type: 'string',
                  title: '{{t("Trusted email domains")}}',
                  'x-component': 'Input',
                  'x-decorator': 'FormItem',
                  'x-decorator-props': {
                    tooltip: '{{t("Optional comma-separated domains, for example example.com, subsidiary.com")}}',
                  },
                },
              },
            },
            advanced: {
              ...
              properties: {
                ...
                postLogoutRedirectUri: {
                  type: 'string',
                  title: '{{t("Post logout redirect URI")}}',
                  'x-component': 'Input',
                  'x-decorator': 'FormItem',
                  'x-decorator-props': {
                    tooltip: '{{t("This URI must be registered at the identity provider.")}}',
                  },
                },
                autoLoginRedirect: {
                  type: 'boolean',
            ...
            buttonStyle: {
              ...
              properties: {
                type: {
                  ...
                  'x-component': 'Select',
                  'x-decorator': 'FormItem',
                  enum: ['default', 'primary', 'dashed', 'link', 'text'].map((v) => ({
                ...
                shape: {
                  ...
                  'x-component': 'Select',
                  'x-decorator': 'FormItem',
                  enum: ['default', 'circle', 'round'].map((v) => ({
                    label: v.charAt(0).toUpperCase() + v.slice(1),
                    value: v,
                  })),
                },
  ...
  () => {
    ...
    const buttonProps = useMemo(() => {
      ...
      if (Array.isArray(customStyleField?.value)) {
        props.style = customStyleField.value.reduce(
          (obj: React.CSSProperties, s: { property: string; value: string }) => {
      ...
      <Card title="Preview" type="inner">
        <Space direction="vertical" style={{ width: '100%' }}>
          <div style={{ display: 'flex', flexDirection: 'column', width: '320px' }}>
            <Button {...buttonProps}>Preview Button</Button>
          </div>
        </Space>
      </Card>
    );
...
export const Options = () => {
  ...
  return (
    <SchemaComponent
      scope={{ t }}
      components={{ Usage, ButtonStylePreview, ArrayItems, Space, FormTab }}
      schema={schema}
    />
  );
};
