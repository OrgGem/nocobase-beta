...
import { generatePgpKey } from '../services/pgp';
...
function createFakeApp() {
  ...
  return { app: app as unknown as Application, keys };
}
...
describe('crypto-adapter (AES-256-GCM)', () => {
  ...
  it('round-trips the JSON wire format envelope', async () => {
    ...
    const envelope = JSON.parse(encrypted.body.toString('utf8')) as { encoding: string; ciphertext: string };
