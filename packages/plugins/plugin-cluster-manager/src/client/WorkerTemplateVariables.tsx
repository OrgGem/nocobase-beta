...
/**
 * Shared (runtime-agnostic) screen: works in both the legacy v1 settings page
 * and the v2 FlowEngine UI because it only relies on useApp() from
 * @nocobase/client-v2, which both runtimes provide.
 */
export default function WorkerTemplateVariables() {
  ...
  const t = useCallback((key: string) => app.i18n.t(key, { ns: 'cluster-manager' }), [app.i18n]);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [variablesResponse, stacksResponse] = await Promise.all([
        api.request({ url: 'workerTemplate:list', method: 'get' }),
        api.request({ url: 'orchestratorStacks:list', method: 'get', params: { pageSize: 100 } }),
      ]);
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      // antd already surfaces field-level errors; avoid a misleading
      // "failed to save" toast on pure validation failures.
      return;
    }
    setSaving(true);
    try {
      await api.request({ url: 'workerTemplate:upsert', method: 'post', data: values });
  ...
  const remove = useCallback(
    async (variable: WorkerTemplateVariable) => {
      try {
        await api.request({ url: 'workerTemplate:destroy', method: 'post', data: { id: variable.id } });
