...
/**
 * Cache-management UI. Lists rows in `carboneRenderCache` and lets an admin
...
export const CacheTab: React.FC = () => {
  ...
  const onDropRow = async (row: CacheRow) => {
    try {
      const r: any = await api.resource(COLLECTION.renderCache).invalidate({ values: { cacheKey: row.cacheKey } });
  ...
  const onDropTemplate = async (templateId: number) => {
    try {
      const r: any = await api.resource(COLLECTION.renderCache).invalidate({ values: { templateId } });
              ...
              <Space wrap>
                ...
                {row.templateId != null ? (
                  <Popconfirm
                    ...
                    onConfirm={() => onDropTemplate(Number(row.templateId))}
                  >
