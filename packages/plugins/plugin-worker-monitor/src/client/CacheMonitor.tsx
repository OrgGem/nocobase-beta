...
export function CacheMonitor() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [storesRes, cachesRes, memRes] = await Promise.all([
        api.request({ url: 'workerMonitorCacheMgr:stores' }),
        api.request({ url: 'workerMonitorCacheMgr:caches' }),
        api.request({ url: 'workerMonitorCacheMgr:redisMemory' }),
      ]);
  ...
  const handleFlushAll = async () => {
    try {
      await api.request({ url: 'workerMonitorCacheMgr:flushAll', method: 'post' });
