...
export const BusinessMonitor: React.FC = () => {
  ...
  const [tracingTarget, setTracingTarget] = useState<
    | { kind: 'job'; id?: string | number; jobKey?: string }
    | { kind: 'queueItem'; id: string | number }
    | { kind: 'log'; log: RowData }
    | null
  >(null);
  ...
  const openTrace = (kind: 'job' | 'queue' | 'log', record: RowData) => {
    ...
    if (kind === 'job') {
      setTracingTarget({
        ...
        jobKey:
          typeof record.Key === 'string' ? record.Key : typeof record.JobKey === 'string' ? record.JobKey : undefined,
      });
      return;
    }
    if (kind === 'queue' && (typeof id === 'string' || typeof id === 'number')) {
      setTracingTarget({ kind: 'queueItem', id });
      return;
    }
    if (kind === 'log') {
      setTracingTarget({ kind, log: record });
    }
  };
  ...
  return (
    <div>
                                    ...
                                    columns={[
                                      ...
                                      { title: t('Machine'), dataIndex: 'HostMachineName', ellipsis: true },
                                      {
                                        ...
                                        render: (_: unknown, row: RowData) => (
                                          <Button size="small" onClick={() => openTrace('job', row)}>
                                            {t('Trace')}
                                          </Button>
                                        ),
                                      },
                                  ...
                                  <Table
                                    ...
                                    columns={[
                                      ...
                                      { title: t('Priority'), dataIndex: 'Priority', width: 100 },
                                      {
                                        ...
                                        width: 100,
                                        render: (_: unknown, row: RowData) => (
                                          <Button size="small" onClick={() => openTrace('queue', row)}>
                                            {t('Trace')}
                                          </Button>
                                        ),
                                      },
                                  ...
                                  <Table
                                    ...
                                    columns={[
                                      ...
                                      { title: t('Message'), dataIndex: 'Message', ellipsis: true },
                                      {
                                        ...
                                        width: 100,
                                        render: (_: unknown, row: RowData) => (
                                          <Button size="small" onClick={() => openTrace('log', row)}>
                                            {t('Trace')}
                                          </Button>
                                        ),
                                      },
      ...
      <Drawer
        title={t('Execution Trace')}
        open={Boolean(tracingTarget)}
