...
import { NextAppRoutesProvider, useNextAppRoutes } from './NextAppRoutesProvider';
...
export const MenuPermissions: React.FC<{
  ...
  const { routeList } = useNextAppRoutes();
  ...
  const resource = api.resource('roles.nextAppRoutes', role.name);
...
export const NextAppAllRoutesProvider: React.FC<{ active: boolean }> = ({ active }) => {
  const { data, loading } = useRequest(
    {
      resource: 'nextAppConfig',
      action: 'list',
      params: {
        filter: { enabled: true },
        sort: ['createdAt'],
  ...
  if (apps.length === 0) {
    return (
      <div style={{ padding: 20, textAlign: 'center' }}>
        {t('No apps configured. Create an app in the Routes section first.')}
      </div>
    );
  }
      ...
      {apps.map((app) => (
        <div key={app.id}>
          <h3 style={{ marginBottom: 16 }}>
            {app.title || app.path} {t('routes')}
          </h3>
          <NextAppRoutesProvider action="list" configId={app.id} manual={false}>
            <MenuPermissions active={active} />
          </NextAppRoutesProvider>
        </div>
      ))}
    </div>
  );
