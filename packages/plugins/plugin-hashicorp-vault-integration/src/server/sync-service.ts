  ...
  private async doTick(): Promise<void> {
    ...
    for (const { connection, mappings: groupMappings } of groups.values()) {
      ...
      for (const mapping of groupMappings) {
        ...
        try {
          const value = await client.readSecret(
            mapping.get('secretPath') as string,
            mapping.get('secretKey') as string,
          );
          this.cache.set(variableKey, value);
          if (mapping.get('syncToEnv')) {
            this.plugin.app.environment.setVariable(variableKey, value);
            this.cache.markEnvKey(variableKey);
          } else if (this.cache.isEnvKey(variableKey)) {
            this.plugin.app.environment.removeVariable(variableKey);
            this.cache.unmarkEnvKey(variableKey);
          }
          anySuccess = true;
