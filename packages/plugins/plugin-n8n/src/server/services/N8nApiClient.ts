export class N8nApiClient {
  ...
  private async request(path: string, options: { method?: string; body?: any; timeout?: number } = {}) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error(`[plugin-n8n] n8n API error: ${method} ${url} → ${res.status}: ${text}`);
  ...
  async updateVariable(id: string, data: { key?: string; value?: string }) {
    return this.request(`/api/v1/variables/${this.safeId(id)}`, { method: 'PUT', body: data });
  }
  ...
  // Projects
  async listProjects(cursor?: string, limit = 100) {
    const params = new URLSearchParams({ limit: String(limit) });
    if (cursor) params.set('cursor', cursor);
  ...
  async listAllProjects() {
    ...
    let pages = 0;
    do {
      const res = await this.listProjects(cursor);
      all.push(...(res.data || []));
      cursor = res.nextCursor;
      pages++;
  ...
  async getCredentialSchema(typeName: string) {
    return this.request(`/api/v1/credentials/schema/${encodeURIComponent(typeName)}`);
  }
