...
import { SharedFormMessageContext } from '../components/SharedFormPage';

export const useSharedSubmitActionProps = () => {
  const form = useForm();
  ...
  const { setShowMessage } = useContext(SharedFormMessageContext);
  return {
    type: 'primary',
    async onClick() {
