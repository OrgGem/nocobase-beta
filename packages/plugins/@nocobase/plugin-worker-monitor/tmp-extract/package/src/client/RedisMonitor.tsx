  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [infoRes, pubsubRes, clientsRes, slowlogRes] = await Promise.all([
        api.request({ url: 'workerMonitorRedis:info' }),
        api.request({ url: 'workerMonitorRedis:pubsub' }),
        api.request({ url: 'workerMonitorRedis:clients' }),
        api.request({ url: 'workerMonitorRedis:slowlog', params: { count: 10 } }),
      ]);
      setInfo(infoRes.data);
      setChannels(pubsubRes.data?.channels || {});
      setTotalChannels(pubsubRes.data?.total_channels || 0);
      setClients(clientsRes.data?.data || []);
      setSlowlog(slowlogRes.data?.data || []);
    } catch (err: any) {
