...
/**
 * DocPixie Plugin — Prompt Templates
 *
 * Ported from docpixie/ai/prompts.py (477 lines).
 * All AI prompts are centralized here — DO NOT embed prompts in service logic.
 *
 * Template variables use {variable} syntax, replaced at runtime via string interpolation.
 */
...
export const SYSTEM_DOCPIXIE = `You are DocPixie, an AI assistant that helps users understand and analyze their documents.
...
export const SYSTEM_VISION_EXPERT =
  'You are a document analysis expert using vision capabilities to analyze document images.';

export const SYSTEM_DIRECT_ANSWER = `You are DocPixie, a helpful AI assistant.
Answer the user's question directly without referring to any documents.
Be concise and accurate.`;

export const SYSTEM_SYNTHESIS = `You are DocPixie, an expert at synthesizing complex document analysis results.
...
export const SYSTEM_SUMMARIZER = 'You are a helpful assistant that creates concise summaries.';

export const SYSTEM_QUERY_REFORMULATOR = 'You are a query reformulation expert.';

export const SYSTEM_QUERY_CLASSIFIER = 'You are a query classification expert. Always respond with valid JSON.';

export const SYSTEM_TASK_PLANNER = `You are an expert task planning assistant specializing in breaking down complex queries.
You understand how to decompose multi-faceted questions into simpler, focused tasks that can be executed independently.`;

export const SYSTEM_ADAPTIVE_PLANNER = `You are an adaptive task planning agent. Based on new information you gather, you can modify your task plan by adding new tasks, removing unnecessary tasks, or updating existing ones. You are pragmatic and efficient - you stop when you have enough information to answer the user's query.`;

export const SYSTEM_PAGE_SELECTOR = `You are a document page selection expert. You analyze document summaries and page information to select the most relevant pages for answering specific questions using vision analysis.`;

// =============================================================================
// TASK PROCESSING
// =============================================================================

export const TASK_PROCESSING_PROMPT = `You are DocPixie, analyzing specific documents to complete a focused task as part of a larger analysis.
...
export const SYNTHESIS_PROMPT = `You are DocPixie. Your job is to answer the user's specific question using the analysis results provided.
...
export const ADAPTIVE_INITIAL_PLANNING_PROMPT = `You are creating an initial task plan for a document analysis query. Create the MINIMUM number of tasks (1-3) needed to gather distinct information to answer the user's question.
...
// =============================================================================
// ADAPTIVE PLAN UPDATE — Core "Adaptive" feature
// =============================================================================

export const ADAPTIVE_PLAN_UPDATE_PROMPT = `You are an adaptive agent updating your task plan based on new information. Analyze what you've learned and decide if you need to modify your remaining tasks.
...
export const VISION_PAGE_SELECTION_PROMPT = `Analyze these document page images and select the most relevant pages for this query:
...
export const TEXT_PAGE_SELECTION_PROMPT = `Given these page summaries, select the most relevant pages for this task:
...
export const QUERY_REFORMULATION_PROMPT = `You are a query reformulation expert. Your task is to resolve references in the current query to make it suitable for document search.
...
export const QUERY_CLASSIFICATION_PROMPT = `Analyze the user's query and determine if it needs document retrieval to answer.
...
export const CONVERSATION_SUMMARIZATION_PROMPT = `Summarize the following conversation, focusing on:
 ...
 * Replaces {variable_name} with values from the params object.
 *
 * @example
 * ```ts
 * const filled = fillPrompt(SYNTHESIS_PROMPT, {
 *   original_query: 'What is Q3 revenue?',
 *   results_text: '...',
 * });
 * ```
 */
export function fillPrompt(template: string, params: Record<string, string>): string {
  // Use a single-pass replacement keyed on a lookup map to avoid creating
  // one RegExp object per parameter per call.
  const lookup = new Map(Object.entries(params));
  return template.replace(/\{([^}]+)\}/g, (match, key) => {
    return lookup.has(key) ? lookup.get(key)! : match;
  });
}
