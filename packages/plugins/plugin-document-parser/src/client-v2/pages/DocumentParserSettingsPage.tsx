...
type Pipeline = {
  pdf: { enabled: boolean; textThreshold: { minCharacters: number }; maxBytes: number; maxPages: number };
  ocr: { enabled: boolean; primary: OcrConfig; fallback: OcrConfig; timeoutMs: number };
...
type Provider = { id: string | number; title: string; enabled: boolean };
type RuntimeStatus = {
...
export default function DocumentParserSettingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [settingsResponse, runtimeResponse] = await Promise.all([
        ctx.api.request({ url: 'docParserSettings:get', method: 'get' }),
        ctx.api.request({ url: 'docParser:getRuntime', method: 'get' }),
      ]);
      form.setFieldsValue(toFormValues(unwrapData<Settings>(settingsResponse, defaultSettings())));
  ...
  const checkEngine = async (engine: 'pdf-inspector' | 'markitdown') => {
    ...
    try {
      const response = await ctx.api.request({ url: 'docParser:checkEngine', method: 'post', data: { engine } });
  ...
  return (
    <section aria-labelledby="document-parser-settings-heading" style={{ maxWidth: 860, padding: 24 }}>
      <Typography.Title id="document-parser-settings-heading" level={3}>
        {t('Document Parser')}
      </Typography.Title>
      <Alert
        showIcon
      ...
      <Form form={form} layout="vertical">
        ...
        <Card title={t('OCR Providers')} style={{ marginTop: 16 }}>
          <ProviderManager onProvidersChanged={setProviders} />
        </Card>
        <Button type="primary" icon={<SaveOutlined />} loading={saving} onClick={save} style={{ marginTop: 16 }}>
          {t('Save')}
        </Button>
      </Form>
    </section>
...
function OcrRouteFields({ prefix, kind, providerOptions, form, t, label }: OcrRouteFieldsProps) {
  ...
  return (
    <>
      <Form.Item name={`${prefix}Kind` as keyof FormValues} label={label}>
        <Select
          style={{ maxWidth: 420 }}
      ...
      {kind === 'external-provider' ? (
        <Form.Item
          ...
          label={t(`${title} external OCR provider`)}
