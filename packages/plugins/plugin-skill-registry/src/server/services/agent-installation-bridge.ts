...
export class AgentInstallationBridge {
  ...
  async rollback(installationId: string | number, installedById?: string | number) {
    const target = await serviceFor(this.pluginManager).getRollbackTarget(installationId);
    if (!target) {
      throw new RegistryError(
    ...
    return this.install(target.registryVersionId, target.updatePolicy, installedById);
  }
}
